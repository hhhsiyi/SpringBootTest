...
//            List<User> userList = userDao.getUserList();//
    ...
    public void test2() {
        SqlSession sqlSession = MyBatisUtils.getSqlSession();
        UserDao userDao = sqlSession.getMapper(UserDao.class);
        User1 user1List = userDao.getUserById(1);
        System.out.println(user1List);
        sqlSession.close();
    }
    ...
    public void test3() {
        SqlSession sqlSession = MyBatisUtils.getSqlSession();
        UserDao userDao = sqlSession.getMapper(UserDao.class);
        int userList = userDao.addUser(new User1(5, "格纹", "19980928"));
        if (userList > 0) {
    ...
    public void test4() {
        SqlSession sqlSession = MyBatisUtils.getSqlSession();
        UserDao userDao = sqlSession.getMapper(UserDao.class);
        int userList = userDao.updateUser(new User1(1, "真何文", "19980928"));
        if (userList > 0) {
            System.out.println(userList);
            sqlSession.commit();
        }
    ...
    public void test7() {
        ...
        User1 user1List = userDao.getUserById2(userMap);
        if (user1List !=null) {
            System.out.println(user1List);
            sqlSession.commit();
        }
    ...
    public void test8() {
        SqlSession sqlSession = MyBatisUtils.getSqlSession();
        UserDao userDao = sqlSession.getMapper(UserDao.class);
        List<User1> user1List = userDao.getUserLike("h");//这种情况有点low啊
        if (user1List !=null) {
            System.out.println(user1List);
            sqlSession.commit();
        }
